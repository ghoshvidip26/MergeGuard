...
import { fetchRepoOwner, fetchAllRepoFiles, getGithubRepoSummary, } from "../RAG/Data.js";
...
async function buildRepoDocuments() {
    ...
    const allFiles = (await fetchAllRepoFiles(repoOwner.owner, repoOwner.repo, repoOwner.currentBranch)).filter((f) => !f.path.startsWith("dist/") && !f.path.endsWith(".d.ts"));
    const repoDetails = await getGithubRepoSummary(repoOwner.owner, repoOwner.repo);
    const documents = [];
    // Repo-level context
...
export async function splitDoc(rawDocs) {
    return await textSplitter.splitDocuments(rawDocs);
}
const vectorStore = new MemoryVectorStore(embeddings);
export async function addDocIntoVectorStore(rawDocs) {
...
export async function call(question) {
