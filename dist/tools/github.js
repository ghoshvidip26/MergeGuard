...
export const getGithubRepoSummary = tool(async ({ githubOwner, repoName }) => {
    try {
    ...
    description: "Get a summary of a GitHub repository, including open issues, pull requests, and recent commits.",
    schema: z.object({
        githubOwner: z.string(),
        repoName: z.string(),
    }),
});
export const getCommitsWithFiles = tool(async ({ owner, repo, limit = 10 }) => {
    ...
    description: "Return recent commits along with the list of files changed in each commit.",
    schema: z.object({
        owner: z.string(),
        repo: z.string(),
...
export const getGithubFileContent = tool(async ({ githubOwner, repoName, filePath, ref }) => {
    try {
    ...
    description: "Fetch and return the decoded content of a specific file from a GitHub repository.",
    schema: z.object({
        githubOwner: z.string(),
        repoName: z.string(),
        filePath: z.string(),
...
export const listLocalFiles = tool(async ({ dirPath }) => {
    try {
