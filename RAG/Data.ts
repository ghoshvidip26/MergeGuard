...
async function getBranchTreeSha(
  ...
  repo: string,
  branch: string
): Promise<{ treeSha: string; resolvedBranch: string }> {
  ...
  if (!branchExists) {
    console.warn(
      `⚠️  Branch '${branch}' not found on remote. Falling back to 'main'.`
    );
  }
  ...
  for (let i = 0; i < codeFiles.length; i += batchSize) {
    ...
    const results = await Promise.allSettled(
      batch.map(async (f) => {
        ...
        if (!isGithubFile(data)) return null;
...
export async function getGithubRepoSummary(owner: string, repo: string) {
